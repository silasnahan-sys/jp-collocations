...
export interface LibraryEntry {
  blockId: string;
  noteClass: NoteClass;
  file: string;              // the reconciled/anchored file the callout lives in
  note: string;
  reconciled: string;
...
export function renderAnchoredFile(results: ReconciledResult[], opts: AnchorFileOptions = {}): string {
  ...
  out.push('> 各カラー = ノート種別（🟡セリフ 🟢連語 🔵修辞連語 🩵修辞構文 🔴談話）。ライブラリで種別を変更できます。', '');
  results.forEach((r, i) => {
...
export function retypeInMarkdown(md: string, blockId: string, newClass: NoteClass): string {
  ...
  // find the callout block whose trailing ^blockId matches; swap its [!keyword] and leading emoji
  const re = new RegExp(`(> \\[!)([a-z-]+)(\\][+-]? )(?:🟡|🟢|🔵|🩵|🔴) ([\\s\\S]*?\\n\\^${blockId}\\b)`);
